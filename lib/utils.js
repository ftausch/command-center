...
export function projectHealthScore(project, tasks = []) {
  ...
  // Don't flag overdue if all tasks are already done (just needs archiving)
  const allTasksDone = tasks.length > 0 && openTasks.length === 0;
  if (daysLeft !== null && daysLeft < 0 && project?.status !== 'Done' && !allTasksDone) {
